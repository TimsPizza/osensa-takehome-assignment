<script lang="ts">
	...
	let overloadDurationSeconds = $state(10);
	let overloadOrdersPerSecond = $state(100);
	let overloadObservationSeconds = $state(10);
	...
	const overloadActive = $derived(
		$overload.status === 'running' || $overload.status === 'observing'
	);
	const anyTestActive = $derived(
		burstActive || idempotencyActive || saturationActive || overloadActive
	);
	...
	function startOverload(): void {
		overloadDurationSeconds = clampInteger(overloadDurationSeconds, 5, 30);
		overloadOrdersPerSecond = clampInteger(overloadOrdersPerSecond, 20, 200);
		overloadObservationSeconds = clampInteger(overloadObservationSeconds, 5, 60);
		void controller.startOverload({
			durationSeconds: overloadDurationSeconds,
			ordersPerSecond: overloadOrdersPerSecond,
			observationSeconds: overloadObservationSeconds
		});
	}
			...
			<div class="grid gap-4 sm:grid-cols-3">
				<div class="space-y-2">
					...
					<Input
						...
						min="5"
						max="30"
						bind:value={overloadDurationSeconds}
					/>
				</div>
				<div class="space-y-2">
					...
					<Input
						...
						min="20"
						max="200"
						bind:value={overloadOrdersPerSecond}
					/>
				</div>
				<div class="space-y-2">
					...
					<Input
						...
						min="5"
						max="60"
						bind:value={overloadObservationSeconds}
					/>
				</div>
			</div>
